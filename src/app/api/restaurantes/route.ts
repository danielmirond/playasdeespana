...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ restaurantes }, {
    headers: {
      // Un array vacío suele ser un fallo puntual de Overpass: si se cachea
      // 24h en el CDN, la sección queda "sin datos" un día entero aunque la
      // fuente se recupere. Vacío → reintento en 2 min.
      'Cache-Control': restaurantes.length > 0 ? 'public, s-maxage=86400, stale-while-revalidate=172800' : 'public, s-maxage=120, stale-while-revalidate=300',
    },
  })
}
