...
export default async function Page({ params }: Props) {
  ...
  return (<><Nav /><main style={{maxWidth:1000,margin:'0 auto',padding:'2rem 1.5rem 5rem'}}>
    ...
    <div style={{background:'var(--card-bg)',border:'1.5px solid var(--line)',borderRadius:20,overflow:'hidden',marginBottom:'2rem'}}><MapaPlayas playas={top.map(x=>x.p)} height="400px" /></div>
    <ol style={{margin:0,padding:0,listStyle:'none',display:'flex',flexDirection:'column',gap:'.55rem'}}>
      {top.map(({p,sc},i) => {
        ...
        return <li key={p.slug}><Link href={`/en/beaches/${p.slug}`} style={{display:'flex',alignItems:'center',gap:'1rem',background:'var(--card-bg)',border:'1.5px solid var(--line)',borderRadius:14,padding:'1rem 1.15rem',textDecoration:'none'}}>
