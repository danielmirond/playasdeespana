...
export const metadata: Metadata = { title: 'Top 10 Best Beaches by Coast | Spain Rankings', description: 'Rankings of the 10 best beaches on each coast of Spain.', alternates: { canonical: '/en/top', languages: {  'es': '/top', 'en': '/en/top', 'x-default': '/top' } } }
export default function Page() {
  return (<><Nav /><main style={{maxWidth:1000,margin:'0 auto',padding:'2rem 1.5rem 5rem'}}>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:'.65rem'}}>
      {COSTAS.map(c => (
        <Link key={c.slug} href={`/en/top/${c.slug}`} style={{display:'flex',alignItems:'center',gap:'.75rem',padding:'1rem 1.15rem',background:'var(--card-bg)',border:'1px solid var(--line)',borderLeft:`4px solid ${c.color==='#f8fafc'?'var(--accent)':c.color}`,borderRadius:6,textDecoration:'none'}}>
