...
export const metadata: Metadata = { title: 'Accessible Beaches in Spain | PMR List', description: 'Beaches with wheelchair access, ramps, amphibious chairs and adapted facilities in Spain.', alternates: { canonical: '/en/accessible-beaches', languages: {  'es': '/playas-accesibles', 'en': '/en/accessible-beaches', 'x-default': '/playas-accesibles' } } }
...
export default async function Page() {
  ...
  return (<><Nav /><main style={{maxWidth:1000,margin:'0 auto',padding:'2rem 1.5rem 5rem'}}>
    ...
    {playas.filter(p => p.lat && p.lng).length >= 6 && (
      <section aria-labelledby="top-acc-en" style={{marginBottom:'2rem'}}>
        <h2 id="top-acc-en" style={{fontFamily:'var(--font-serif)',fontSize:'1.45rem',fontWeight:700,color:'var(--ink)',marginBottom:'1rem'}}>Top 6 accessible beaches</h2>
        <TopBeachCardsConHero
          ...
          limit={6}
          eyebrow={`Top 6 · ${playas.length} accessible beaches`}
        />
      </section>
    )}
