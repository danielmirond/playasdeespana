...
export default async function Page() {
  ...
  return (<><Nav /><main style={{maxWidth:1000,margin:'0 auto',padding:'2rem 1.5rem 5rem'}}>
    ...
    <Link href="/en/routes/configure" style={{display:'inline-flex',alignItems:'center',gap:'.4rem',background:'var(--accent)',color:'#fff',padding:'.75rem 1.25rem',borderRadius:12,fontSize:'.92rem',fontWeight:800,textDecoration:'none',marginBottom:'2rem'}}>🛣️ Build Your Own Route</Link>
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:'.75rem'}}>
      {rutas.map(r => (
        <Link key={r.slug} href={`/en/routes/${r.slug}`} style={{display:'flex',flexDirection:'column',background:'var(--card-bg)',border:'1.5px solid var(--line)',borderLeft:`4px solid ${r.costa.color}`,borderRadius:16,padding:'1.1rem 1.2rem',textDecoration:'none'}}>
