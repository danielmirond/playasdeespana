...
export default async function Page({ params }: Props) {
  ...
  return (<><Nav /><main style={{maxWidth:1000,margin:'0 auto',padding:'2rem 1.5rem 5rem'}}>
    ...
    <a href={ruta.googleMapsUrl} target="_blank" rel="noopener noreferrer" style={{display:'inline-flex',alignItems:'center',gap:'.4rem',background:'var(--accent)',color:'#fff',padding:'.75rem 1.25rem',borderRadius:4,fontSize:'.92rem',fontWeight:800,textDecoration:'none',marginBottom:'2rem'}}>🗺️ Open in Google Maps</a>
    <div style={{background:'var(--card-bg)',border:'1px solid var(--line)',borderRadius:6,overflow:'hidden',marginBottom:'2rem'}}><MapaPlayas playas={ruta.paradas.map(p=>p.playa)} height="400px" /></div>
    <ol style={{margin:0,padding:0,listStyle:'none',display:'flex',flexDirection:'column',gap:'.55rem'}}>
      {ruta.paradas.map((p,i) => (
        <li key={p.playa.slug}><Link href={`/en/beaches/${p.playa.slug}`} style={{display:'flex',alignItems:'center',gap:'1rem',background:'var(--card-bg)',border:'1px solid var(--line)',borderRadius:6,padding:'1rem',textDecoration:'none'}}>
          ...
          <span style={{background:p.score>=75?'var(--excelente)':p.score>=55?'var(--aceptable)':'var(--limitado)',color:'#fff',fontWeight:900,padding:'.2rem .4rem',borderRadius:6}}>{p.score}</span>
        </Link></li>
      ))}
    </ol>
  </main></>)
