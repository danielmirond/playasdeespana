...
export default async function Page() {
  ...
  return (<><Nav /><main style={{maxWidth:1000,margin:'0 auto',padding:'2rem 1.5rem 5rem'}}>
    ...
    <div style={{background:'var(--card-bg)',border:'1.5px solid var(--line)',borderRadius:20,overflow:'hidden',marginBottom:'2rem'}}><MapaPlayas playas={playas} height="400px" /></div>
    <div style={{display:'flex',flexDirection:'column',gap:'.5rem'}}>
      {playas.slice(0,20).map(p => (
        <Link key={p.slug} href={`/en/beaches/${p.slug}`} style={{display:'flex',alignItems:'center',gap:'.75rem',padding:'.85rem 1rem',background:'var(--card-bg)',border:'1.5px solid var(--line)',borderRadius:12,textDecoration:'none'}}>
