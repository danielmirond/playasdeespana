// src/app/yoga-playa/[provincia]/page.tsx — Estudios reales de yoga y
// pilates de una provincia costera, con valoración de Google, reseñas,
// playa más cercana de nuestra guía y deep-link a Maps por place_id.
// Datos del sidecar (scripts/build-yoga-estudios.mjs); cero llamadas a
// Places en runtime.
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!d) return {}
  return {
    title: `Yoga y pilates en ${d.provincia}: estudios cerca de la playa`,
    description: `Estudios reales de yoga y pilates en ${d.provincia} con valoraciones y reseñas de Google, y la playa que le queda al lado a cada uno. Esterilla al amanecer, baño después.`,
    alternates: { canonical: `/yoga-playa/${provincia}` },
    openGraph: { type: 'website', url: `${BASE}/yoga-playa/${provincia}` },
  }
}

const dist = (m: number) => (m < 1000 ? `${m} m` : `${(m / 1000).toFixed(1).replace('.', ',')} km`)

export default async function YogaProvinciaPage({ params }: Props) {
  const { provincia } = await params
  const d = DATA[provincia]
  if (!d) notFound()
  ...
  const itemList = {
    ...
    '@type': 'ItemList',
    name: `Estudios de yoga y pilates en ${d.provincia}`,
    ...
    <>
      ...
      <main style={{ maxWidth: 1000, margin: '0 auto', padding: '2rem 1.5rem 5rem' }}>
        <nav style={{ display: 'flex', gap: '.4rem', fontSize: '.75rem', color: 'var(--muted)', marginBottom: '.85rem' }} aria-label="Ruta de navegación">
          ...
          <Link href="/yoga-playa">Yoga y pilates</Link><span aria-hidden="true">›</span>
        ...
        <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(1.6rem,4vw,2.3rem)', fontWeight: 900, color: 'var(--ink)', marginBottom: '.35rem' }}>
          Yoga y pilates en <em style={{ fontWeight: 500, color: 'var(--accent)' }}>{d.provincia}</em>
        </h1>
        <p style={{ fontSize: '.95rem', color: 'var(--ink)', lineHeight: 1.65, margin: '0 0 2rem', maxWidth: 640 }}>
          {d.estudios.length} estudios reales cerca de la costa de {d.provincia}, ordenados por número de
          reseñas. Junto a cada uno, la playa de nuestra guía que le queda más cerca.
        </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '.6rem', maxWidth: 800, marginTop: '2.5rem' }}>
          ...
          <Link href="/yoga-playa" style={{ background: 'var(--card-bg)', border: '1px solid var(--line)', borderRadius: 6, padding: '.85rem 1rem', textDecoration: 'none' }}>
            ...
            <span style={{ fontSize: '.74rem', color: 'var(--muted)' }}>Yoga y pilates en toda la costa.</span>
          </Link>
