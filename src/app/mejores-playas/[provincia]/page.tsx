            ...
            <li key={p.slug}>
              <Link href={`/playas/${p.slug}`} style={{
                ...
                <div style={{ display: 'flex', gap: '.25rem', flexShrink: 0, flexWrap: 'wrap' }}>
                  ...
                  {p.accesible && <span style={{ fontSize: '.6rem', fontWeight: 500, color: 'var(--excelente)', background: 'color-mix(in srgb, var(--excelente) 8%, transparent)', padding: '.12rem .35rem', borderRadius: 100 }}>PMR</span>}
                </div>
              </Link>
            </li>
          ))}
