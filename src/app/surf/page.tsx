...
function surfQuality(m: SurfMeteo): { label: string; color: string; desc: string } {
  if (m.olas >= 1.0 && m.olas <= 2.5 && m.viento <= 20 && m.periodo >= 8)
    return { label: 'EPIC', color: 'var(--excelente)', desc: 'Condiciones ideales para surfear' }
  if (m.olas >= 0.8 && m.olas <= 3.0 && m.viento <= 25)
    return { label: 'BUENO', color: 'var(--mar-500)', desc: 'Buen día de surf' }
  if (m.olas >= 0.5 && m.viento <= 35)
    return { label: 'REGULAR', color: 'var(--aceptable)', desc: 'Se puede surfear con condiciones' }
  if (m.olas < 0.5)
    return { label: 'FLAT', color: '#9ca3af', desc: 'Mar plano, sin olas' }
  return { label: 'HOLD', color: 'var(--noapto)', desc: 'Demasiado fuerte o ventoso' }
}
