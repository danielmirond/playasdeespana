  ...
  return (
    ...
    >
      <div style={{ display:'flex', alignItems:'flex-start', gap:'.75rem' }}>
        <span style={{ flexShrink:0, display:'flex', alignItems:'center' }} aria-hidden="true"><ShieldCheck size={18} weight="bold" color="var(--accent,#b06820)"/></span>
      ...
      <div style={{ display:'flex', gap:'.5rem', justifyContent:'flex-end' }}>
        <button
          ...
          style={{ padding:'.45rem 1rem', borderRadius:'100px', border:'1.5px solid #e8dcc8', background:'transparent', color:'#8a7560', fontSize:'.78rem', fontWeight:600, cursor:'pointer' }}
        >{i18n.rechazar}</button>
        <button
          type="button"
          onClick={() => { localStorage.setItem('cookie_consent', 'all'); setVisible(false) }}
          aria-label={aLabelAceptar}
          style={{ padding:'.45rem 1rem', borderRadius:'100px', border:'none', background:'#b06820', color:'#fff', fontSize:'.78rem', fontWeight:700, cursor:'pointer' }}
        >{i18n.aceptar}</button>
      </div>
    </div>
  )
