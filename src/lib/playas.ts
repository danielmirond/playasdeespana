...
// ────────────────────────────────────────────────────────────────────────
// Helpers para /playas-nudistas. Mismo patrón que perros.

export const getPlayasNudistas = cache(async (): Promise<Playa[]> => {
  const playas = await getPlayas()
  return playas.filter(p => p.nudista === true)
})

export type NudistasStats = PerrosStats // misma forma: total + comunidades + provincias + municipios

export const getNudistasStats = cache(async (): Promise<NudistasStats> => {
  const playas = await getPlayasNudistas()
  const byCom = new Map<string, number>()
  const byProv = new Map<string, { count: number; comunidad: string }>()
  const byMun = new Map<string, { count: number; provincia: string }>()
  for (const p of playas) {
