#!/usr/bin/env node
// scripts/build-chiringuitos.mjs
//
// Sidecar de chiringuitos REALES por provincia costera (Google Places Text
// Search) para revivir /chiringuitos/[provincia], que era plantilla sin un
// solo chiringuito. Son negocios hiperlocales: muestreamos más puntos del
// litoral que en yoga (5 por provincia) y exigimos más oferta (≥5).
//
...
// Uso:  node scripts/build-chiringuitos.mjs      (necesita GOOGLE_PLACES_API_KEY
...
const OUT = resolve(ROOT, 'src/data/chiringuitos.json')
...
const MIN_ESTUDIOS = 5     // gate de calidad por provincia
const MIN_RESENAS = 5      // fuera fichas fantasma
const RADIUS_M = 12000
const PUNTOS_POR_PROV = 5
...
async function searchText(lat, lng) {
  const res = await fetch('https://places.googleapis.com/v1/places:searchText', {
    ...
    body: JSON.stringify({
      textQuery: 'chiringuito de playa',
      maxResultCount: 15,
...
async function main() {
  ...
  for (const [provincia, lista] of [...porProv.entries()].sort()) {
    ...
    const paso = Math.max(1, Math.floor(ordenadas.length / (PUNTOS_POR_PROV - 1)))
    const puntos = []
    for (let i = 0; i < ordenadas.length && puntos.length < PUNTOS_POR_PROV; i += paso) puntos.push(ordenadas[i])
